...
static GstFlowReturn gst_camerasrc_read_preview(GstCameraSrc *camerasrc, GstBuffer **buffer)
{
	...
	for (i = 0 ; i < _MAX_TRIAL_WAIT_FRAME ; i++) {
		/* Wait frame */
		GST_INFO("                camerasrc_wait_frame_available");
	...
	/* Buffer DQ */
	GST_INFO_OBJECT(camerasrc, "                camerasrc_dequeue_buffer");
...
static GstFlowReturn gst_camerasrc_read(GstCameraSrc *camerasrc, GstBuffer **buffer)
{
	...
	if (command == GST_CAMERA_CONTROL_CAPTURE_COMMAND_START) {
		GST_INFO("        gst_camerasrc_capture_start");
	...
	case VIDEO_IN_MODE_VIDEO:
		GST_INFO("            gst_camerasrc_read_preview");
		ret = gst_camerasrc_read_preview(camerasrc, buffer);
		break;
	case VIDEO_IN_MODE_CAPTURE:
		GST_INFO( "        gst_camerasrc_read_capture");
...
static void gst_camerasrc_buffer_finalize(GstCameraBuffer *buffer)
{
	...
	if (camerasrc->cap_fourcc == GST_MAKE_FOURCC('J','P','E','G') &&
		...
		if (capture_data) {
			...
			GST_INFO("        camerasrc_extract_exif_info_from_capture_data");
			camerasrc_extract_exif_info_from_capture_data(camerasrc->v4l2_handle, capture_data);
		...
		GST_INFO("        gst_camerasrc_jpeg_capture");
		ret = gst_camerasrc_jpeg_capture(camerasrc, index, buffer);
		if (ret == FALSE) {
			GST_ERROR("gst_camerasrc_jpeg_capture failed");
		...
		if (capture_done) {
			...
			GST_INFO("count %d", camerasrc->cap_count_current);
		}
	}
	...
	if (camerasrc->buffer_running) {
		if (strcmp(camerasrc->format_name, "ITLV") == 0 && capture_done) {
			GST_INFO("Do NOT queue buffer here. It will be queued after capture.");
		} else {
			/* Buffer Q again */
...
static gboolean gst_camerasrc_jpeg_capture(GstCameraSrc *camerasrc, gint buffer_index, GstCameraBuffer *buffer)
{
	...
	/* Get endoer information */
	if (info.sw_support == -1) {
		CLEAR(info);
		GST_INFO("            camsrcjpegenc_get_info");
		camsrcjpegenc_get_info(&info);
	}

	/* Get src_fmt value using fourcc */
	GST_INFO("            camsrcjpegenc_get_src_fmt");
	camsrcjpegenc_get_src_fmt(MAKE_FOURCC_FROM_STRING(camerasrc->format_name), &enc_param.src_fmt);

	if (enc_param.src_fmt != COLOR_FORMAT_NOT_SUPPORT) {
		/* Check H/W encoder */
		if (info.hw_support) {
			for (i = 0 ; i < info.hw_enc.input_fmt_num ; i++) {
				if (enc_param.src_fmt == info.hw_enc.input_fmt_list[i]) {
					do_encode = TRUE;
					break;
				}
			}

			/* Set enc_type and src_data for H/W JPEG encoding */
			if (do_encode) {
				if (info.hw_enc.mem_addr_type == MEMORY_ADDRESS_PHYSICAL) {
					frame_data.index = buffer_index;
					GST_INFO("            camerasrc_get_frame_data");
					camerasrc_get_frame_data(camerasrc->v4l2_handle, &frame_data);
					enc_param.src_data = (unsigned char *)frame_data.phyAddrY;
					enc_param.src_len = gst_buffer_get_size(buffer->buffer);
				} else {  /* MEMORY_ADDRESS_VIRTUAL */
					if (strcmp(camerasrc->format_name, "SN12") == 0 ||
					    strcmp(camerasrc->format_name, "SN21") == 0 ||
					    strcmp(camerasrc->format_name, "S420") == 0) {
						gst_buffer_map(buffer->buffer, &buf_info, GST_MAP_READ);
						GST_INFO("            _gst_camerasrc_get_normal_buffer");
						ret = _gst_camerasrc_get_normal_buffer(camerasrc, MAKE_FOURCC_FROM_STRING(camerasrc->format_name),
						                                       buf_info.data, camerasrc->width, camerasrc->height,
						                                       &buf_memory);
					    gst_buffer_unmap(buffer->buffer, &buf_info);
						if (ret == FALSE) {
							GST_WARNING_OBJECT(camerasrc, "failed to get normal buffer to encode");
							return FALSE;
						}
						gst_memory_map(buf_memory, &buf_info, GST_MAP_READ);
						enc_param.src_data = buf_info.data;
						enc_param.src_len = buf_info.maxsize;
						gst_memory_unmap(buf_memory, &buf_info);
					} else {
					    gst_buffer_map(buffer->buffer, &buf_info, GST_MAP_READ);
					    enc_param.src_data = buf_info.data;
						enc_param.src_len = buf_info.size;
						gst_buffer_unmap(buffer->buffer, &buf_info);
					}
				}

				enc_type = JPEG_ENCODER_HARDWARE;
				GST_INFO_OBJECT(camerasrc, "Use H/W JPEG encoder");
			} else {
				GST_WARNING_OBJECT(camerasrc, "H/W JPEG encoder NOT support [%d] format",
				                              enc_param.src_fmt);
			}
		}

		/* Check S/W encoder if H/W encoder not supported */
		if (!do_encode && info.sw_support) {
			for (i = 0 ; i < info.sw_enc.input_fmt_num ; i++) {
				if (enc_param.src_fmt == info.sw_enc.input_fmt_list[i]) {
					do_encode = TRUE;
					break;
				}
			}

			/* Set enc_type and src_data for S/W JPEG encoding */
			if (do_encode) {
				if (strcmp(camerasrc->format_name, "SN12") == 0 ||
				    strcmp(camerasrc->format_name, "SN21") == 0 ||
				    strcmp(camerasrc->format_name, "S420") == 0) {
				...
				GST_INFO_OBJECT(camerasrc, "Use S/W JPEG encoder");
			} else {
				GST_WARNING_OBJECT(camerasrc, "S/W JPEG encoder NOT support [%d] format", enc_param.src_fmt );
			}
		}

		if (do_encode) {
			enc_param.width = camerasrc->width;
			enc_param.height = camerasrc->height;
			enc_param.jpeg_mode = JPEG_MODE_BASELINE;
			enc_param.jpeg_quality = camerasrc->cap_jpg_quality;

			GST_INFO("            camsrcjpegenc_encode");
			ret = camsrcjpegenc_encode(&info, enc_type, &enc_param);

			/* Release allocated memory */
			gst_buffer_map(buffer->buffer, &buf_info, GST_MAP_READ);
			if (enc_param.src_data != (unsigned char *)frame_data.phyAddrY &&
			    enc_param.src_data != buf_info.data) {
				if (enc_param.src_data) {
					free(enc_param.src_data);
					enc_param.src_data = NULL;
					enc_param.src_len = 0;
				}
			}
			gst_buffer_unmap(buffer->buffer, &buf_info);
			...
			if (ret == CAMSRC_JPEGENC_ERROR_NONE) {
				buf_cap_signal_main = gst_buffer_new_wrapped_full(0,
				                                                  enc_param.result_data,
				                                                  enc_param.result_len,
				                                                  0,
				                                                  enc_param.result_len,
				                                                  NULL,
				                                                  NULL);
				...
				/*call signal*/
				GST_DEBUG_OBJECT(camerasrc, "CALL: capture callback(data:%p, size:%d)",
				                            enc_param.result_data, enc_param.result_len);
				...
				if (enc_param.result_data) {
					free(enc_param.result_data);
					enc_param.result_data = NULL;
					enc_param.result_len = 0;
				}

				camerasrc->cap_count_current++;
				GST_DEBUG_OBJECT(camerasrc, "RETURN: capture callback");

				return TRUE;
			} else {
				GST_ERROR_OBJECT(camerasrc, "JPEG encode failed");
			}
		} else {
			GST_WARNING_OBJECT(camerasrc, "No encoder support format [%d]", enc_param.src_fmt);
		}
	} else {
		GST_WARNING_OBJECT(camerasrc, "Not supported fourcc");
	}
...
void gst_camerasrc_set_capture_command(GstCameraSrc *camerasrc, GstCameraControlCaptureCommand cmd)
{
	...
	if (camerasrc->cap_fourcc == GST_MAKE_FOURCC('J','P','E','G')) {
		...
		if (strcmp(camerasrc->format_name, "ITLV") == 0) {
			GST_INFO_OBJECT(camerasrc, "Interleaved preview mode");
		} else {
			GST_INFO_OBJECT(camerasrc, "Other format zero shutter lag capture - size can be only same as preview size");
		}

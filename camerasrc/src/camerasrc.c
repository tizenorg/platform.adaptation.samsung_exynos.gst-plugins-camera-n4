...
int _camerasrc_ioctl_s_parm(camerasrc_handle_t *p, int fd, int type);
...
int _camerasrc_ioctl_qbuf(camerasrc_handle_t *p, int fd, int type, int memory, int idx, camerasrc_buffer_t *buffer)
{
	...
	camsrc_info("ENTER - fd %d, type %d, memory %d, idx %d, buffer %p, planes %d",
	...
	for (i = 0 ; i < MAX_PLANE_NUM ; i++) {
		if (buffer->planes[i].bo) {
			...
			camsrc_info("[index %d] [plane %d] fd %d, length %d",
...
int _camerasrc_ioctl_dqbuf(camerasrc_handle_t *p, int fd, int type, int memory, int *idx, int num_planes)
{
	...
	camsrc_info("ENTER - fd %d, type %d, memory %d, num planes %d", fd, type, memory, num_planes);
...
static int _camerasrc_dequeue_buffer(camerasrc_handle_t *handle, int *buf_index, camerasrc_buffer_t *buffer, camerasrc_buffer_t *thm_buffer)
{
	...
	camsrc_info("[SENSOR] DQBUF index %d", buf_index_sensor);
	...
	_camerasrc_set_shot_meta(p, GRP_3AA, -1, timestamp, shot_sensor);
	...
	shot_sensor = (struct camera2_shot_ext *)p->buffer_isp[buf_index_sensor].planes[1].start;
	_camerasrc_set_shot_meta(p, GRP_ISP, -1, timestamp, shot_isp);
	...
	camsrc_info("SCP DQBUF index : %d", buf_index_scp);

	memcpy(buffer, &p->buffer_scp[buf_index_scp], sizeof(camerasrc_buffer_t));

	camsrc_info("bo compare %p, %p", buffer->planes[0].bo, p->buffer_scp[buf_index_scp].planes[0].bo);
...
int camerasrc_create_buffer(camsrc_handle_t handle)
{
	...
	p->format_scc.num_buffers = p->format_scp.num_buffers - 8;
...
int camerasrc_start_preview_stream(camsrc_handle_t handle)
{
	...
	err = _camerasrc_ioctl_s_parm(p, p->fd_sensor, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
	if (err != CAMERASRC_SUCCESS) {
		strerror_r(p->errnum, err_msg, CAMERASRC_ERRMSG_MAX_LEN);
	...
	memset(&subdev_format, 0x0, sizeof(struct v4l2_mbus_framefmt));
	...
	if (p->buffer_sensor) {
		for (i = 0 ; i < p->format_sensor.num_buffers ; i++) {
			err = _camerasrc_set_shot_meta(p, GRP_SENSOR, -1, 0, (struct camera2_shot_ext *)p->buffer_sensor[i].planes[1].start);
			if (err != CAMERASRC_SUCCESS) {
...
int camerasrc_stop_stream(camsrc_handle_t handle)
{
	...
	if (p->cur_dev_id == CAMERASRC_DEV_ID_SECONDARY &&
		...
		ret = _camerasrc_ioctl_s_ctrl(p, p->fd_sensor, V4L2_CID_IS_S_STREAM, 15); /* 15?? */
		if (ret != CAMERASRC_SUCCESS) {
			strerror_r(p->errnum, err_msg, CAMERASRC_ERRMSG_MAX_LEN);
                                    ...
                                    unsigned long long timestamp, struct camera2_shot_ext *shot_ext)
{
	...
	if (grpid == GRP_3AA) {
		...
		shot_ext->shot.udm.bayer.width = 2650;
	...
	camsrc_info("GRP: %d FCOUNT: %d", grpid, fcount);
